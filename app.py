...
# التعديل الرئيسي: دالة عرض الـ PDF (أكثر استقرار على السيرفر)
def display_pdf(file_path):
    try:
        ...
        # ناخد اسم الملف فقط (مهما كان المسار المحفوظ قديم)
        filename = os.path.basename(file_path)
        actual_path = os.path.join(UPLOAD_FOLDER, filename)
       
        if not os.path.exists(actual_path):
            st.error(f"الملف غير موجود على السيرفر: {filename}")
            st.info("جرب تحميل الملف من جديد في وضع التعديل.")
            return
        
        with open(actual_path, "rb") as f:
            pdf_bytes = f.read()
        
        # زرار التحميل (شغال 100%)
        st.download_button(
            ...
            key=f"dl_{filename}_{hash(file_path)}"
        )
        
        st.info("⚠️ عرض الـ PDF داخل الصفحة بيشتغل أفضل على **Firefox**. على Chrome غالبًا بيحظر.")
