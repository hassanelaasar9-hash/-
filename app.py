...
# الدالة المعدلة لفتح الملف في تبويب جديد
def display_pdf(file_path):
    try:
        ...
        if os.path.exists(actual_path):
            ...
            # كود جافا سكريبت لفتح الـ PDF في نافذة جديدة مباشرة
            pdf_link = f'data:application/pdf;base64,{base64_pdf}'
            st.markdown(f'<a id="pdf_link" href="{pdf_link}" target="_blank" style="display:none;"></a>', unsafe_allow_html=True)
            st.components.v1.html(f"""
                <script>
                    window.parent.document.getElementById('pdf_link').click();
                </script>
            """, height=0)
            st.success(f"✅ جاري فتح التقرير: {filename}")
        else:
