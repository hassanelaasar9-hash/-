...
def display_pdf(file_name):
    """عرض ملف PDF مباشرة في الصفحة بدون تحميل إجباري"""
    try:
        if not file_name:
            st.warning("⚠️ لا يوجد ملف مرفق")
            return
        ...
        # عرض PDF مباشر - مش هيطلب تحميل
        pdf_viewer = f'''
        <div style="width:100%; height:800px; margin:20px 0; border-radius:10px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.1);">
            <embed 
...
with tab2:
    ...
    if not df_raw.empty:
        ...
        if selected_rows:
            ...
            # عرض الملف مباشرة هنا - مجرد عرض، مش تحميل
            if row['file_name']:
                st.markdown("### 📄 التقرير المرفق")
                display_pdf(row['file_name'])
                st.markdown("---")
            else:
                st.info("📭 لا يوجد تقرير مرفق لهذه المعاينة")
