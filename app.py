...
def auto_backup():
    """عمل نسخة احتياطية تلقائية بعد كل إضافة"""
    try:
...
with tab4:
    ...
    with col4:
        ...
        if backups:
            st.info(f"📁 يوجد {len(backups)} نسخة احتياطية يدوية")
            for backup in sorted(backups, reverse=True)[:5]:
                st.write(f"- {backup}")
        else:
            st.info("لا توجد نسخ احتياطية يدوية حتى الآن")
...
with tab1:
    ...
    with st.form("reg_form"):
        ...
        with c1:
            ...
            phone2 = st.text_input("رقم التليفون الثاني (اختياري)")
